...
if(isset($_POST['submit'])){
   ...
   $select_user = $conn->prepare("SELECT * FROM `posts` WHERE id = ? LIMIT 1");
   ...
   if(!empty($name)){
      $update_name = $conn->prepare("UPDATE `posts` SET title = ? WHERE id = ?");
      $update_name->execute([$name, $user_id]);
      $success_msg[] = 'Username updated!';
   }

   if(!empty($email)){
      $verify_email = $conn->prepare("SELECT * FROM `posts` WHERE email = ?");
      $verify_email->execute([$email]);
      if($verify_email->rowCount() > 0){
         $warning_msg[] = 'Email already taken!';
      }else{
         $update_email = $conn->prepare("UPDATE `posts` SET email = ? WHERE id = ?");
  ...
  if($empty_old != 1){
      ...
      if($verify_old_pass == 1){
         if($c_pass == 1){
            if($empty_new != 1){
               $update_pass = $conn->prepare("UPDATE `posts` SET password = ? WHERE id = ?");
...
if(isset($_POST['delete_image'])){
   ...
   $select_old_pic = $conn->prepare("SELECT * FROM `posts` WHERE id = ? LIMIT 1");
   ...
   if($fetch_old_pic['image'] == ''){
      ...
      $update_old_pic = $conn->prepare("UPDATE `posts` SET image = ? WHERE id = ?");
...
<section class="account-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="text" name="name" maxlength="50" placeholder="<?= $fetch_profile['title']; ?>" class="box">
